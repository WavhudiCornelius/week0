...
export default function ProfilesPage(props){
    ...
    var ProfileItems = props.developers.map(developer => (
            <li  onClick={() => props.viewProfilePage({name: developer.username, id: developer._id})} key={developer._id}>
                <NavLink to={`/${developer.username}/${developer._id}`}>
                    <span className="name">{developer.username}</span>
